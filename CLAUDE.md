...
- 求人数、利用者数、満足度、口コミ、許可番号、会社所在地、代表者、プライバシーポリシー情報は要確認項目として扱う。
  ...
  - **確認済み（2026-08-31 オーナー回答）**：会社名 XCHANGE株式会社（英文 XCHANGE Inc.）/ 代表者 柴山 友貴 / 設立 2022年2月 / 所在地 〒104-0061 東京都中央区銀座1丁目22番11号 銀座大竹ビジデンス2階 / 電話 03-6693-5450 / メール info@xchange-inc.com / 事業内容 有料職業紹介・RPO・評価制度設計・M&A仲介 / 許可番号 13-ユ-316946。
    この確認の過程で、**プライバシーポリシー3本の所在地・代表者・問い合わせ先が別会社（and-and.jp）のテンプレート由来の値のままだった**ことが判明し訂正した。法定表示が誤ったまま公開されていた期間がある。
    会社情報は現在8ページ（privacypolicy 3本 + service 5本）に直書きされているので、`scripts/check-company-info.mjs` が全ページの一致と旧値の残存を見張る。値を変えるときはこのスクリプトの `TRUTH` も直すこと。
- ダミー情報を本番用として扱わない。
...
- **ブランチへの push ≠ 本番反映**：本番デプロイ（GitHub Actions `Deploy to Xserver`）は **`main` への push でのみ起動**する（`workflow_dispatch` 手動を除く）。作業ブランチ（例 `claude/*`）に push しただけ・PRを作っただけでは、本番（`denkilp.builders-job.com`）は更新されない。
  ...
  - **【2026-08-31 オーナー指示】マージ／PR作成の前は、毎回この内容を出して確認を取る。返事をもらうまで実行しない。**
    「全体をQAして push commit マージ」のようにまとめて指示された場合も、**マージの直前でいったん止めて**挟む。
    オーナーは中身を把握していない前提なので、長くなっても詳細を省かないこと。

    ```
    ■ 対象ブランチと、その分岐元（main から切ったのか、別ブランチの上に載っているのか）
    ■ 変更ファイル一覧（全件）と行数
    ■ 本番に出るファイル / 出ないファイルの内訳
       （rsync除外の .github/ scripts/ gas-recorder/ deploy/ v2-deploy/ docs/ *.md は本番に出ない）
    ■ 本番の見た目・フォーム・計測が変わるか。変わるなら具体的に何が
    ■ STG実機確認をしたか（表示に関わるなら必須）
    ■ ロールバック方法
    ```

    背景: 複数のブランチを並行で持つ運用になり、「別の作業のついでに、意図していないものまで
    main に入る」ことをオーナーが懸念している。ブランチを main から切って独立させるだけでは
    足りず、**マージの瞬間に何が入るのかを毎回言葉にする**こと。
