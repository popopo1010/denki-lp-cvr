...
- **ダークモード端末でも視認性が崩れないこと**（2026-06-12 に視認性事故2回目。詳細は `docs/release-incidents.md`）。
  - LP には `color-scheme: only light` と `html, body { background-color: #fff }` を必ず維持する。
  - 半透明・グラデ背景の sticky / fixed 要素は、背面が白以外でもボタンが読める設計にする。
  - リリース前に `node scripts/check-denkikouji-release.mjs`（ダークモード回帰チェック込み）を必ず通し、スマホ目視はライト/ダーク両モードで行う。
