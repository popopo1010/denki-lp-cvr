...
- **【頻出バグ 2026-09-06】氏名／生まれ年（step05）の入力中にエラー帯が出る（オーナー実機・denkikouji-nd STG）**：姓を1文字打った瞬間に「お名前を入力してください」の赤帯が名前ラベルを覆い、生まれ年を打っている間も名前のエラーが出続けた。原因は `initNameInputs` の判定が「触れた項目が1つでもあれば、足りない項目のエラーを即出す」で、**いま入力中の項目に対して叱っていた**こと（2026-08-29 に「CTAが無効の間は理由を出す」へ戻したときの副作用。共通 app.js なので denkikouji 本番でも同じだった）。ルール3点：(1) **フォーカス中の項目のエラーは出さない**（`document.activeElement` を見る）(2) 出すときは項目を特定する（「お名前（名）も入力してください」「生まれ年（西暦）は…」）(3) フォーカスが外れて CTA が無効なら必ず理由を出す（2026-08-29 の原則は維持）。blur は `setTimeout(...,0)` で**次のフォーカス先が確定してから**判定する（blur 時点は activeElement=body なので、姓→名へ移る瞬間に名前エラーが一瞬出る）。`check-form-invariants`（配線）と `e2e-lp-flow-local`（実キーボードで再現。value 代入＋input イベントでは activeElement が動かず再現しない）が見張る。**入力ステップのバリデーションを触ったら「入力中に叱っていないか」を実機で確認**。
