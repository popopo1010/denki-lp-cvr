...
- 実行タイミング: 作業ブランチ・PR = `.github/workflows/ci.yml`、`main`/`staging` = `deploy.yml`、手元一括 = `scripts/release-pre-check.sh`。
