...
| CE - thanks_job_preview_view | `thanks_job_preview_view` |
| CE - thanks_job_preview_cta | `thanks_job_preview_cta` |
| CE - thanks_job_intent_select | `thanks_job_intent_select` |
| CE - thanks_job_card_click | `thanks_job_card_click` |
...
| `thanks_jobs_view` | `CE - thanks_job_preview_view` | `preview_count`, `user_pref` |
| `thanks_jobs_cta` | `CE - thanks_job_preview_cta` | `scroll_target` |
| `thanks_jobs_intent` | `CE - thanks_job_intent_select` | `job_intent` |
| `thanks_jobs_card` | `CE - thanks_job_card_click` | `job_id`, `job_intent` |

ダッシュボード設計: `docs/thanks-v2-dashboard.md`
...
2. `dk_lp` の `denkikouji` / `sekokanri` `main.js` を静的LPに反映（任意）
3. GTM プレビュー: LP送信 → `lead_form_submit` → サンクスで `thanks_page_view` + `lead_conversion`
