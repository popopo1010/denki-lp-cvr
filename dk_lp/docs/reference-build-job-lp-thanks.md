...
- [`reference-kinoshita-index.md`](reference-kinoshita-index.md) — 木下リファレンス索引（ここから辿る）
- [`reference-kinoshita-marketing.md`](reference-kinoshita-marketing.md) — 木下勝寿（北の達人）マーケ・LPノウハウ
...
| 予約完了表示 | Bで大きく日時表示 | 枠内 `t-booking-done--hero` |
| 第2CV | 任意・事前アンケート | 予約後に LINE ブロック表示 |
| 紹介シェア | なし | 削除済み（不要） |

### 予約 → LINE フロー（実装済み）

1. LP送信 → `/denki-lp-cvr/thanks-v2/?lp=...`（`sessionStorage._tel` 保存）
2. 初回: `body.is-awaiting-booking` で `#line-section` 非表示
3. 枠タップ → 予約 → `calendar_booked` → ご予約完了UI → `thanks_line_step_revealed` → LINE表示・スクロール
4. 再訪: `dk_booking_done` があれば最初から LINE まで表示

GTM: `docs/GTM-thanks-setup.md`  
JS: `assets/js/thanks-booking-custom.js`, `assets/js/thanks-gtm.js`

**リポジトリ上の自社サンクス**

- 新: `施工管理LP/thanks-v2/index.html`
...
- GTM: `docs/GTM-thanks-setup.md`
- 独自予約: `施工管理LP/gas-recorder/独自予約セットアップ.md`
- 本番手順: `施工管理LP/本番反映手順書.md`（v2 URL一覧）
...
| 2026-06-02 | 予約→LINEフロー・thanks-v2遷移・GTMイベントを追記 |
