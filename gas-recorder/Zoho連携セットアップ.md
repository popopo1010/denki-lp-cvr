...
- **二重処理だけを止める**：作成前に**電話番号でZoho側を検索**するが、重複とみなすのは
  **直近24時間以内に作られた商談だけ**（`ZOHO_DEDUP_HOURS`）。それより古い商談しか無い場合は
  **新しい商談を作る**——過去に登録した人の再登録は、営業にとって「今また動いている人」なので、
  新規リードとして上げる必要がある（2026-08-29 修正。以前は同じ番号が1件でもあれば作らず、
  再登録がSlackにもZohoにも現れない事故になっていた）。
  営業が手で作った商談とぶつからない。
...
- **項目の最大長を超えると、その1件が丸ごと作られない**。Zoho は項目ごとに上限を持ち、
  1文字でも超えると 400 `INVALID_DATA` で**商談の作成ごと拒否する**。
  2026-09-07 本番で発生: 施工管理LPの step01 は資格の複数選択で、11個すべて選んだ人の
  商談名「氏名/資格×11」が **142文字**になり `Deal_Name`(120) を超えた。
  **Slack通知もシート記録も正常に出る**ため、商談が無いことに後から気づけないのが厄介。
  対策は3段構え（`zoho.js`。1つ壊れても次段が拾う）:
  1. `zohoBuildDealName()` — 商談名を意味の単位で詰める（`姓名/資格A, 資格B ほかN件`）。
     資格の全量は `shikaku` と `lp_info` に残るので情報は失われない。
  2. `zohoClampDealFields()` — 送信直前に**全テキスト項目**をメタデータの最大長で丸める。
     `Deal_Name` 専用にしない（`lp_info` や `shikaku_sonota` も同じ理由で落ちうる）。
  3. `zohoRepairDeal()` — それでも弾かれたら、Zohoが名指しした項目（`details.api_name`）
     だけを短縮/除外して**1回だけ再送**する。必須項目は落とさず短縮のみ。
     修復して作れた場合は `zoho_error` 列に `repaired: …` の痕跡を残す。
  上限値は項目メタデータ（6時間キャッシュ）の `length` から読むので、
  **Zoho画面で項目長を変えてもコード修正は不要**。メタが取れないときだけ既定値を使う。
