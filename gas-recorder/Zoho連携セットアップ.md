...
- **商談名（Deal_Name）は 120 文字が上限**。施工管理LPの step01 は資格の複数選択で、
  全部選ぶと「氏名/資格×11」が 120 文字を超え、`INVALID_DATA maximum_length:120` で
  **商談作成ごと拒否される**（2026-09-07 本番で発生。Slack・シートは正常、商談だけ無い）。
  `zohoBuildDealName()` が収まらないときは先頭の資格だけ残して「ほかN件」を付ける。
  資格の全量は `shikaku`（保有資格）と `lp_info` に入るので商談名を短くしても失われない。
  落ちた行は `zoho_error` にこのエラーが残り `zoho_deal_id` が空なので、
  修正反映後に `backfillZohoDeals()` を1回流せば商談化される。
