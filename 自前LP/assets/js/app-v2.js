(() => {
  ...
  function initNameInputs(group) {
    ...
    function validate() {
      if (allFilled()) {
        ...
        if (errBox) {
          ...
          if (errText) {
            var missing = [];
            var lastN = group.querySelector("#last-name");
            var firstN = group.querySelector("#first-name");
            if (lastN && !lastN.value) missing.push("姓");
            if (firstN && !firstN.value) missing.push("名");
            if (isYearInput) {
              var v = (bdayYearInput.value || "").trim();
              if (!/^[0-9]{4}$/.test(v)) {
            ...
            if (missing.length === 0) errText.textContent = "入力内容をご確認ください";
            else if (missing.length === 1) errText.textContent = missing[0] + "を入力してください";
            else errText.textContent = missing.join("・") + "を入力してください";
          }
        }
        moveIconById("#" + target.id);
      }
    ...
    // 姓+名 が揃ったら生年月日input に同期focus(iOSキーボード自動オープン)
