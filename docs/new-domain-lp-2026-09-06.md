...
## 4b. STG 実機で見つかった不具合と修正（2026-09-06 オーナー報告「名前と生年月日のところでエラーがバグってた」）

- 症状: 姓を1文字打った瞬間に「お名前を入力してください」の赤帯が名前ラベルを覆い、生まれ年を打っている間も名前のエラーが出続ける。
- 原因: 共通 `app.js` の `initNameInputs` が「触れた項目が1つでもあれば、足りない項目のエラーを即出す」判定で、**いま入力中の項目に対して叱っていた**（本番 `denkikouji/` も同じ挙動だった）。
- 2回目の報告（PC）: 名を打っている最中に、まだ触っていない生まれ年のエラーが名前欄の上に出た。「フォーカス中の欄だけ除外」では先の欄を叱ってしまう。
- 修正（3実装＋ミラー）: `app.js` / `app-v2.js` / `dk_lp/denkikouji/main.js` に `pendingMessage()` / `placeErrBox()`。(1) 入力中は [姓,名,年] の順で**通り過ぎた欄だけ**叱る (2) 手が止まったら足りない欄を順に出す (3) 文言は欄を特定し、帯はその欄の上へ移す (4) focus と blur（`setTimeout 0`）で判定し直す。`app.js?v20260906c` / `app-v2.js?v20260906b` / dk_lp `main.js?v=20260906b`。
