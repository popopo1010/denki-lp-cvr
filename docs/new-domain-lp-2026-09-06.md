...
- 修正（3実装＋ミラー）: `app.js` / `app-v2.js` / `dk_lp/denkikouji/main.js` に `pendingMessage()` を入れ、(1) フォーカス中の項目のエラーは出さない (2) 出すときは項目を特定（「お名前（名）も入力してください」等）(3) フォーカスが外れて CTA が無効なら必ず理由を出す。blur は `setTimeout(...,0)` で次のフォーカス先が確定してから判定。`app.js?v20260906b` / `app-v2.js?v20260906a` / dk_lp `main.js?v=20260906a`。
