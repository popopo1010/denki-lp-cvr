...
## 2026-06-24 「しつこい営業なし」微コピー — repo に無いのに繰り返し指摘される

**症状:** オーナーが本番FVで「しつこい営業なし」系の微コピーを見て「まだ入っている／なんで繰り返すのか」と指摘。過去にも同種（「営業電話なし」等の文言除去 #2026-06-23 ③）で取りこぼし扱いされた。

**原因:** その文言は **リポジトリのどのテキストファイルにも存在しない**（`しつこい営業`/`営業`/`勧誘` 全 grep で0件）。本番FVバナーは画像（`first_banner0103.webp/jpg`）に焼き込み＋WPテーマ（`/wp-content/themes/original-thema/`）配下で、**リポジトリのデプロイ最適化対象外**（#2026-06-23 ⑤と同根）。repo の HTML/CSS/JS をいくら直しても本番からは消えないため、「直したのに残る＝繰り返し指摘」のループになる。

**対応/再発防止:**
- 文言変更を頼まれたら **まずリポジトリ全体を grep**。0件なら即「repo 側には無い。FVバナー画像の再生成 or WPテーマ編集が必要」と現在地を返す（repo を触って『直した』と報告しない）。
- FVコピー変更は **画像再生成が前提**（alt だけ変えても画像と不一致／本番不変。#2026-06-23 ⑤）。WPテーマ本体はこのリポジトリに無いため、変更にはテーマ側のアクセス手段が要る。
- このルールは `CLAUDE.md`（denkikouji セクション）に恒久メモとして追記済み。
