...
## 2026-07-08 step06 STEP表示隠れの再々発（6回目）: ユーザータップ起因は autofocus 廃止では防げない

- 症状: 5回目の対策（dk-inappでautofocus廃止）デプロイ後も、**ユーザーが自分で電話入力欄をタップ**してキーボードが開くと同じくSTEP表示・タイトルがバー裏に隠れる（オーナー報告・入力済み「0」で発覚）。
- 学び: ブラウザ主導のスクロールは「フォーカスが発生する限り」起きる。autofocus抑止は入口を1つ塞いだだけだった。
- 対策（3点セットに拡張）: (a) autofocus抑止（既存） (b) `html.dk-inapp input{scroll-margin-top:120px}` をcritical CSSに標準搭載（33ファイル） (c) focusinナッジ＝フォーカス確定300ms後、入力欄がキーボード上に見えることを保証できる場合のみSTEP表示が見える位置までscrollByで戻す（app.js / app-v2.js / dk_lp main.js）。
- 併せて: LandingHubのpicture組み替えによる残存ラボCLS対策として、FVのpictureを固定高さボックス化（サイズはCLAUDE.md参照）。

## 2026-07-08 step06でSTEP表示・タイトルがアプリ内バーの裏に隠れる（5回目・オーナー報告）

- 症状: LINEアプリ内ブラウザでstep06（電話番号）に到達すると、キーボードが自動で開き、電話入力欄が画面最上部に張り付いてSTEP表示・「携帯番号」タイトルが上部半透明バーの裏に隠れる。
- 原因: `showPage` のステップ到達時autofocus。`focus({preventScroll:true})` はJSスクロールを防ぐだけで、**キーボードが開く瞬間のブラウザ主導スクロール（フォーカス欄を可視ビューポートへ寄せる動き）は防げない**。`html.dk-inapp` の padding-top:44px も無力（スクロール自体が起きるため）。
- 修正: **`html.dk-inapp`（LINE/Instagram等）ではステップ到達時の自動フォーカスをしない**。ユーザーが入力欄をタップした時だけキーボードを開く（文脈を読んだ後なので隠れても支障なし）。app.js＋dk_lpミラー＋dk_lp/denkikouji/main.js に適用。
- 教訓: 「上部が隠れる」の犯人クラスは5つに更新（CLAUDE.md 頻出バグ項目参照）。**autofocusとアプリ内ブラウザの組み合わせは構造的に非互換**——preventScrollを付けても安全にならない。
...
## 2026-08-31 テーマCSSスナップショットのワークフローが常時失敗していた

`snapshot-theme-css.yml` の「Create PR if changed」が、行継続のつもりで `\\` と書いていた。

```yaml
  gh pr create --base main --head "$BR" \
    --title "..." \
    --body "..." \\          ← YAML の block scalar は中身をそのまま渡すので、
    || echo "PR already exists"   bash からは「エスケープされたバックスラッシュ」
```

YAML としては妥当なので YAML の検証では捕まらない。壊れ方が厄介で、
**bash はそこまでを実行してから構文エラーで落ちる**：

1. `git push -f origin theme-css-snapshot` は成功する
2. `gh pr create` も走る（末尾に余計な `\` が引数として付いた状態で）
3. そのあと `|| echo` の行が独立コマンドの先頭になり構文エラー → ステップ失敗

つまり「ブランチもPRもできているのにワークフローは赤」という、一番読み飛ばされる形。
さらに `|| echo` のフォールバックが効かないので、2回目以降は
`gh pr create`（PRが既にある）の失敗で `set -e` により必ず落ちる。

このワークフローは毎週月曜 6:00 JST に走り、**WPテーマCSSの変更を全LPへ反映する唯一の経路**
（2026-08-23 に全LPが `theme-snapshot.css` を読むようになって以降）。
常時赤いワークフローは読まれなくなるので、テーマ変更の検知そのものが機能しなくなる。

### 対策

`scripts/check-workflow-shell.mjs` を追加した。全ワークフローの `run:` を取り出して
`bash -n` にかける（`${{ }}` はプレースホルダに潰す）。YAMLパーサに依存しないので
CIに追加の依存を持ち込まない。

同種の事故は同じ日にもう一度起きかけていた——`deploy.yml` の rsync で、
行継続の途中にコメントを置いて除外指定と転送元・転送先ごと消える書き方
（`#` 以降が行連結後に全部消える）。**行継続の周辺は目視で通る種類の壊れ方をする**。

なお、この番人を書いた当日に、その番人のステップ名を
`name: ワークフローの run: をシェルとして構文検査` と書いて YAML を壊した。
`run:` の抽出は正規表現ベースで YAML の妥当性は見ないため、
「シェルは全部OK」と言いながら壊れたYAMLを通してしまう。
クォートしていないスカラーに `": "` が入っていないかの検査も同じ番人に足した。
どちらも意図的に壊して発火を確認している。
