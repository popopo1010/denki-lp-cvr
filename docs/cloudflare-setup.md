...
> **【結論】2026-09-04 に見送り。CLAUDE.md にも同じ結論を書いてある。**
> 再開の条件は「TTFB 0.8秒超 または LCP 2.0秒超」だけ（次の計測目安 2026-12）。
> 条件を満たしていないのに、この手順書があることを理由に導入を提案しない（2026-09-16 に蒸し返した）。
...
## 段階0の結果（2026-09-04 18:59 オーナー計測・PSI 実際のユーザー環境・denkikouji・過去28日）
...
**判断: 段階1・2 は見送り（今は入れない）。** 実ユーザーの数字が既に合格域で、媒体（Google/Meta）が見る
LP速度も問題なし。Cloudflare を入れて得られるのは TTFB 0.7→0.3〜0.4 秒程度で、DNS移行・WAF・クローラー到達の
リスクに見合わない。**3か月ごと（または広告配信量を増やす前）に同じ計測をやり直し、TTFB が 0.8 秒を超えるか
LCP が 2.0 秒を超えたら段階1から再開**する。下の手順はそのときのために残す。
