...
| step06 社会的証明 | 利用者の96.4%が満足と回答 – 34,513人が利用中（**送信ボタンの上**にバッジ表示。数値の根拠は `本番反映手順書.md` 確認事項） |
...
| thanks LINE CTA | hero=「この3件の続きをLINEで受け取る（30秒・無料）」/ jobs=「この求人の続きをLINEで受け取る（無料）」（ティーザー直結・2026-07-02）。section/dock=「LINEで求人全文を受け取る」を維持（検証needle）。全文の解放タイミングは「お電話後」の説明文で担保。PCのみQR表示 |
...
| 禁止 | 本登録・旧 `/thanks/` CVトリガー依存（96.4%・34,513は2026-06に復活、根拠は確認事項） |
