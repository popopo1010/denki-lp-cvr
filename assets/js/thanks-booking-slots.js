/**
 * サンクス予約: 静的JSON → GAS の順で取得（表示を高速化）
 */
(function () {
  ...
  var CACHE_KEY = window.BOOKING_SLOTS_CACHE_KEY || "dk_booking_slots_cache";
  var CACHE_TTL = window.BOOKING_SLOTS_CACHE_TTL_MS || 5 * 60 * 1000;
    ...
    return fetch(url, { credentials: "same-origin" })
      ...
      .then(function (json) {
        ...
        if (json.generated_at) {
          var age = Date.now() - new Date(json.generated_at).getTime();
